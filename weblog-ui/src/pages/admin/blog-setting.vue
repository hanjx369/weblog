...
// 初始化博客设置数据，并渲染到页面上
async function initBlogSettings() {
  // 请求后台接口
  await getBlogSettingsDetail().then((res) => {
    if (res.success) {
      ...
      if (res.data.xituHomepage) {
        isXituChecked.value = true
        form.xituHomepage = res.data.xituHomepage
      }
    }
  })
}
  ...
  <div>
    ...
    <el-card shadow="never">
      <el-form ref="formRef" :model="form" label-width="160px" :rules="rules">
        ...
        <!-- 开启 CSDN 访问 -->
        <el-form-item label="开启稀土掘金访问">
          <el-switch
            v-model="isXituChecked"
            inline-prompt
            :active-icon="Check"
            :inactive-icon="Close"
            @change="xituSwitchChange"
          />
        </el-form-item>
        <el-form-item label="稀土掘金主页访问地址" v-if="isXituChecked">
          <el-input
            v-model="form.xituHomepage"
            clearable
            placeholder="请输入稀土掘金主页访问的 URL"
          />
        </el-form-item>
        <el-form-item>
